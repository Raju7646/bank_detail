...
use ineuron_raju
select *from bank_details
select    min(balance) as minimum_balance from bank_details
...
DELIMITER &&
create procedure avg_bal_jobrole2(IN bank_details varchar(30))
BEGIN
	select  avg(balance) from bank_details where marital = bank_details;
END &&
call sudh()
call avg_bal_jobrole2('single')
create view raju_bank as select age, job,balance from bank_details
select * from raju_bank
delimiter &&
create procedure bank_data(in varchar(30))
BEGIN
select  avg(balance) from bank_details where job = entrepreneur;
END &&
